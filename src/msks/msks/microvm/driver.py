...
class MicrovmDriver(abc.ABC):
    ...
    async def console(
        ...
        ``user`` is given (with the client terminal's size and type in
        ``rows``/``cols``/``term``); legacy images ignore the
        parameters and serve the raw root shell. Backends without an
