...
IP_FAIL_AT = "TEST_IP_FAIL_AT"
IP_STDERR = "TEST_IP_STDERR"
NFT_FAIL_AT = "TEST_NFT_FAIL_AT"
NFT_STDERR = "TEST_NFT_STDERR"

IP_STUB = """#!/bin/sh
printf '%s\n' "$*" >> {log}
fail_at="${{TEST_IP_FAIL_AT:-}}"
if [ -n "$fail_at" ] && [ "$1 $2" = "$fail_at" ]; then
  printf '%s' "${{TEST_IP_STDERR:-boom}}" >&2
  exit 1
fi
exit 0
"""
...
printf '%s\n' "--- $*" >> {log}.stdin
cat >> {log}.stdin
fail_at="${{TEST_NFT_FAIL_AT:-}}"
if [ -n "$fail_at" ] && [ "$1 $2" = "$fail_at" ]; then
  printf '%s' "${{TEST_NFT_STDERR:-boom}}" >&2
  exit 1
fi
exit 0
"""
