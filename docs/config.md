...
| `bootstrap_token` | `MSKSD_BOOTSTRAP_TOKEN` | string | _(unset)_   | Seeds the first bearer token at first boot. The plaintext must fit the HTTP `token` grammar (letters, digits, and the separator characters RFC 9110 allows): websocket authentication carries it in the `Sec-WebSocket-Protocol` handshake (#116), whose value grammar rejects the rest. |
| `access_log`      | `MSKSD_ACCESS_LOG`      | bool   | `false`     | Writes uvicorn's access log — request lines (method, path, query), never headers. Websocket tokens ride the `Sec-WebSocket-Protocol` handshake (#116), not the URL, so the log holds no credentials.                                                                                     |
